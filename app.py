...
def predict_missing_values(df):
    ...
    # Khởi tạo các cột forecasted cho từng biến
    for col in EXPECTED_COLUMNS:
        df[f"{col}_forecasted"] = 0  # 0: không nội suy, 1: đã nội suy
            ...
            for col in missing_cols_list:
                if col in models:
                    try:
                        ...
                        df.at[idx, f"{col}_forecasted"] = 1  # Đánh dấu đã nội suy
    ...
    # Tổng hợp cột is_forecasted và forecasted_columns
    df["is_forecasted"] = df[[f"{col}_forecasted" for col in EXPECTED_COLUMNS]].any(axis=1).astype(int)
    df["forecasted_columns"] = df[[f"{col}_forecasted" for col in EXPECTED_COLUMNS]].apply(
        lambda row: ", ".join([col.replace("_forecasted", "") for col in row.index if row[col] == 1]),
        axis=1
    )
